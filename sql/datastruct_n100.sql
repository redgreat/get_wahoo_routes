...
alter table workout_imps owner to wangcw;
...
alter table workout_imps drop constraint if exists uni_imps_file_name cascade;
alter table workout_imps add constraint uni_imps_file_name unique (file_name);

comment on column workout_imps.id is '自增主键';
comment on column workout_imps.file_name is '手动上传文件名称';
comment on column workout_imps.insert_time is '上传时间';
comment on table workout_imps is '手动导入运动数据信息表';
